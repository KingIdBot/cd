//START
//SUBSCRIBE CHANNEL MRKING GAN
//MAU APA LU GAN
//JANGAN EDIT WOI KALAU LU GAK IZIN DULU
//HAYOO LU LAGI EDIT APA NJIRR
	...
	nzwa.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '18313535216@s.whatsapp.net'
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'memebotLangso':
                                        if (!isRegister) return reply(botLangs.daftarB())
                                        if (isLimit(sender)) return reply(botLangs.limitend(pusname))
					memein = await kagApi.memebotLangso()
				...
				case 'alay':
                    ...
                    if (isLimit(sender)) return reply(botLangs.limitend(pusname))
                    data = await fetchJson(`https://arugaz.my.id/api/edu/corona?country=botLangsonesia}`)
                    reply(data.result)
                    await limitAdd(sender)
                    break
                    case 'quotemaker':
           ...
           break
				...
				case 'ocr':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(botLangs.wait())
						await recognize(media, {lang: 'eng+botLangs', oem: 1, psm: 3})
							.then(teks => {
								reply(teks.trim())
								fs.unlinkSync(media)
							})
	    ...
	    case 'kapankah':
            ...
            break
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                case 'afk':
                                        ...
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *SEDANG AFK ${tels} JANGAN GANGGU GAN*`,
				...
				case 'clone':
					...
					let { jid, id, notify } = groupMembers.fbotLangs(x => x.jid === mentioned)
					try {
					...
					break
                                ...
                                case 'register':
                                        ...
                                        const namaUser = q.substring(0, q.botLangsexOf('|') - 0)
                                        const umurUser = q.substring(q.lastbotLangsexOf('|') + 1)
